...
export default function EditFlagModal({ flag, onClose, onUpdated }: EditFlagModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    excludedUserIds: flag.excludedUserIds?.join(', ') || '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await featureFlagApi.update(flag.id, {
        ...formData,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b">
          <h2 className="text-2xl font-bold text-gray-900">Edit Feature Flag</h2>
          <p className="text-sm text-gray-500 mt-1">Key: {flag.key}</p>
        </div>
